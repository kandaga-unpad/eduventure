		...
		if (!allValid) {
			...
			await fetch('/api/beli-offer', {
				...
				.then(async (res) => {
					...
					await goto('/eduventure/experienceoffer/pembayaran/' + response.kode);
				})
				.catch((err) => {
					console.error(err);
				});
...
<section>
	<a href="/eduventure/experienceoffer" class="text-center mt-10">&leftarrow;Kembali</a>
	...
	<div class="text-center my-5">
		...
		<div class="mb-5">
			<p class="text-green-7">dengan Penawaran Spesial</p>
		</div>
				...
				<tbody>
					{#if $ticketStore.tiketZona1 > 0}
						<tr>
							...
							<td
								>{($ticketStore.tiketZona1 * 315000).toLocaleString('id-ID', {
									style: 'currency',
									currency: 'IDR'
								})}</td
							>
					...
					{#if $ticketStore.tiketZona2 > 0}
						<tr>
							...
							<td
								>{($ticketStore.tiketZona2 * 315000).toLocaleString('id-ID', {
									style: 'currency',
									currency: 'IDR'
								})}</td
							>
					...
					{#if $ticketStore.tiketZona3 > 0}
						<tr>
							...
							<td
								>{($ticketStore.tiketZona3 * 315000).toLocaleString('id-ID', {
									style: 'currency',
									currency: 'IDR'
								})}</td
							>
					...
					<tr>
						...
						<td class="text-right" colspan="2"
							>{(totalTiket * 315000).toLocaleString('id-ID', {
								style: 'currency',
								currency: 'IDR'
							})}</td
						>
