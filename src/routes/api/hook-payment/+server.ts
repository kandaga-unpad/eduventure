...
export const POST: RequestHandler = async ({ request }) => {
	...
	const keysOrQuery: Record<string, any>[] = await (async () => {
		...
		} else if (external_id.startsWith('eduventure-lite')) {
			return await directus.request(
	...
	if (status === 'PAID' && external_id.startsWith('eduventure-tiket')) {
		...
		await directus.request(
			updateItems('tiket_eduventure_experience', allKeys, {
				status_pendaftaran: 'paid',
		...
		if (keysOrQuery[0]?.voucher !== '') {
			getChosenVoucher = await directus.request(
				readItems('voucher_eduventure', {
					filter: {
						status: 'published',
						kode_voucher: {
		...
		if (
			(keysOrQuery[0]?.voucher || keysOrQuery[0]?.voucher !== '') &&
			getChosenVoucher[0]?.tiket.length < getChosenVoucher[0]?.total_kuota
		) {
			const filteredTiket = keysOrQuery.filter(
				(item) => item.voucher === getChosenVoucher[0]?.kode_voucher
			);
			...
			await directus.request(
				updateItem('voucher_eduventure', getChosenVoucher[0].id, {
		...
		await directus.request(
			updateItems('tiket_eduventure_holiyeay', allKeys, {
				status_pendaftaran: 'paid',
				tanggal_pembayaran: paid_at,
				response: body
			})
	...
	} else if (status === 'PAID' && external_id.startsWith('eduventure-lite')) {
		const allKeys = keysOrQuery.map((item) => item.id);
		await directus.request(
			updateItems('pengajuan_eduventure_lite', allKeys, {
	...
	} else if (status === 'EXPIRED' && external_id.startsWith('eduventure-lite')) {
		const allKeys = keysOrQuery.map((item) => item.id);
		await directus.request(
			updateItems('pengajuan_eduventure_lite', allKeys, {
